...
Until now `markWon` created a project `Order` for **every** proposal type — so winning an AMC produced
a construction project with nine build stages. **Gate: tsc 0 · lint 0 new (30 warnings, same as
`main`) · 145 unit (+4) · `next build` clean · new `verify-won-routing` (31 checks, live DB) ·
clients-p0/p1 + proposals-p4/p6/p7 + amc + sell + drawings-p0 green · AMC win browser-driven
end-to-end, zero console errors.**

- **`markWon` now branches on `proposalType`** and returns a discriminated `MarkWonResult`, so every
  caller is forced by the type system to handle all three outcomes rather than assuming a project:
  - Project / BOQ → `Order` + 9 stages + budget + SITE location + milestones — **unchanged**
  - AMC Proposal → `ServiceContract` with its preventive-maintenance visits generated by the same
    `generateVisitSchedule` the manual AMC form uses
  - Service Proposal → `ServiceTicket` with an SLA derived from the quoted priority
  All three still find-or-create the `Client` on the same exact-customerName key and record the win
  for the A14 learning loop. The Won button now names what it will actually create.
- **The figure that would have been wrong: `annualValue` seeds from `subtotal`, not `grandTotal`.**
  `ServiceContract.annualValue` is a pre-GST field by design (the v28 GST audit confirmed this and
  deliberately left AMC alone), while a proposal's grand total includes 18%. Seeding from the grand
  total would have inflated every contract — and the Service module's recurring-revenue run-rate — by
  18%. Asserted explicitly in `verify-won-routing`. A service **ticket** takes the GST-inclusive grand
  total instead, because that is what the customer agreed to pay.
- **Per-type fields the new records are built from** — `amcProposalDataSchema` (term, frequency,
  visits/year, and a scope block mirroring `ServiceContract.scope`'s shape so it copies without
  translation) and `serviceProposalDataSchema` (job description, priority), each with its own wizard
  step. Without these there'd be nothing to derive a contract term or visit schedule from. Defaults
  (12 months / quarterly / 4 visits) apply when an AMC is won with them unfilled.
- **`ServiceTicket.value` — tickets had no money field at all.** Winning a ₹50,000 service job used to
  record the revenue nowhere. Sell-side (like `BOQItem.rate`), so NOT an admin-only stripped key.
- **Client lifetime value counts all three modules.** New `realisedValue()`/`realisedProposals()` in
  `proposal-pick.ts` take a won proposal's order value, contract value or job value — exactly one is
  ever set, since each link is `@unique` on the proposal, so they cannot double-count. Extends the
  per-proposal iteration built in v44 rather than adding a parallel path. `clientStats.activeProjects`
  deliberately still counts only orders: the Service dashboard has its own live-contract count and
  folding it in would double-report.
  ⚠️ **Receivables, GST and collections are untouched** — those read `Invoice` and track money actually
  *billed*. Conflating "sold" with "billed" would corrupt figures used for tax filing.
- **Bug the verification caught**: the "already won" guard only covered the Order path, so winning an
  AMC twice hit the `proposalId` unique constraint and surfaced a raw Prisma P2002. All three paths now
  return the existing record with `already: true`.
- **Flaw fixed in `verify-drawings-p0` itself**: it defaulted to `localhost:3000`, which on this
  machine is a *different application* — it answered `/api/healthz` and the file-gate checks silently
  verified the wrong app. It now fingerprints the healthz body (`checks` + `automations`) and skips
  with a clear message rather than testing something else.
- **Migrations are additive**: three nullable columns (`ServiceContract.proposalId`,
  `ServiceTicket.proposalId`/`value`) plus their indexes and FKs. The `@unique` on `proposalId` is safe
  on populated tables — existing rows get NULL and Postgres permits unlimited NULLs in a unique index.
  FKs are `ON DELETE SET NULL`, so removing a proposal never deletes a live contract or an open job.
