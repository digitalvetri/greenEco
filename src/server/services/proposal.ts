...
export async function markWon(
  ...
  const proposal = await prisma.proposal.findFirst({
    ...
    include: {
      ...
      order: true,
      // Needed for the same "already won" guard the order path has — without these,
      // winning an AMC twice hits the proposalId unique constraint and surfaces a raw
      // Prisma P2002 instead of quietly returning what already exists.
      contract: { select: { id: true, contractNo: true, visits: { select: { id: true } } } },
  ...
  if (!proposal) throw new Error("Proposal not found");
  if (proposal.order) return { kind: "ORDER" as const, orderId: proposal.order.id, already: true };
  const version = currentVersionOf(proposal)!;
  if (!version.approvedById) throw new Error("Approve the proposal before marking it Won");

  // What a win PRODUCES depends on what was sold. An AMC becomes a service contract
  // with a visit schedule; a one-off service job becomes a ticket; only an actual
  // plant becomes a project with execution stages and payment milestones. Before this,
  // winning an AMC created a construction project with nine build stages.
  if (proposal.proposalType === "AMC Proposal") {
    if (proposal.contract) {
      return {
        kind: "CONTRACT" as const,
        contractId: proposal.contract.id,
        contractNo: proposal.contract.contractNo,
        visits: proposal.contract.visits.length,
        already: true,
      };
    }
    return winAsAmcContract(ctx, proposal, version);
  }
  if (proposal.proposalType === "Service Proposal") {
    if (proposal.ticket) {
      return {
        kind: "TICKET" as const,
        ticketId: proposal.ticket.id,
        ticketNo: proposal.ticket.ticketNo,
        already: true,
      };
    }
    return winAsServiceTicket(ctx, proposal, version);
  }
  ...
  return prisma.$transaction(async (tx) => {
    ...
    return { kind: "ORDER" as const, orderId: order.id, orderNo, already: false };
  });
}

/**
 * Narrow a win result to the project case. For callers that KNOW they won a Project or
 * BOQ proposal (verification scripts, the Won→Order flow) and would otherwise have to
 * hand-narrow the union at every use. Throws with a clear message if the proposal
 * turned out to be an AMC or service job instead.
 */
export function expectOrder(res: MarkWonResult): { orderId: string; orderNo?: string; already: boolean } {
  if (res.kind !== "ORDER") {
    throw new Error(`Expected this proposal to create a project, but it created a ${res.kind.toLowerCase()}`);
  }
  return res;
}

/** The three things a won proposal can become. Callers switch on `kind` to route. */
export type MarkWonResult =
  | { kind: "ORDER"; orderId: string; orderNo?: string; already: boolean }
  | { kind: "CONTRACT"; contractId: string; contractNo: string; visits: number; already: boolean }
  | { kind: "TICKET"; ticketId: string; ticketNo: string; already: boolean };

/**
 * Find-or-create the Client for a won proposal — the SAME exact-customerName identity
 * key `markWon`'s order path uses, so an AMC and a project for one customer attach to
 * one client row rather than duplicating them.
 */
async function clientForWin(
  tx: Prisma.TransactionClient,
  ctx: Ctx,
  lead: { customerName: string; phone: string; email: string | null; address: string; state: string | null },
) {
  return (
    (await tx.client.findFirst({ where: { companyId: ctx.companyId, name: lead.customerName } })) ??
    (await tx.client.create({
      data: {
        companyId: ctx.companyId,
...
/**
 * A won AMC Proposal becomes a real ServiceContract, with its preventive-maintenance
 * visits generated by the same `generateVisitSchedule` the manual AMC form uses.
 *
 * ⚠️ `annualValue` is seeded from **subtotal, not grandTotal**. ServiceContract.annualValue
 * is a pre-GST figure by design (confirmed in the v28 GST audit, which deliberately left
 * AMC alone); seeding it from the GST-inclusive grand total would inflate every contract
 * and therefore the recurring-revenue run-rate by 18%.
 */
async function winAsAmcContract(
  ctx: Ctx,
  proposal: { id: string; siteAddress: string; lead: { customerName: string; phone: string; email: string | null; address: string; state: string | null }; versions: unknown[] },
  version: { subtotal: Prisma.Decimal; grandTotal: Prisma.Decimal; documentData: Prisma.JsonValue },
): Promise<MarkWonResult> {
  const { generateVisitSchedule } = await import("@/lib/domain/amc");
  const { asAmcProposalData, DEFAULT_AMC_TERM_MONTHS, DEFAULT_AMC_FREQUENCY, DEFAULT_AMC_VISITS_PER_YEAR } =
    await import("@/lib/domain/proposal-document");

  const doc = asAmcProposalData(version.documentData);
  const termMonths = doc.termMonths ?? DEFAULT_AMC_TERM_MONTHS;
  const frequency = doc.frequency ?? DEFAULT_AMC_FREQUENCY;
  const visitsPerYear = doc.visitsPerYear ?? DEFAULT_AMC_VISITS_PER_YEAR;

  const startDate = new Date();
  const endDate = new Date(startDate);
  endDate.setMonth(endDate.getMonth() + termMonths);
  const schedule = generateVisitSchedule(startDate, endDate, visitsPerYear);
  const year = startDate.getFullYear();

  return prisma.$transaction(async (tx) => {
    const contractNo = await allocateNumber(tx, ctx.companyId, "AMC", year);
    await clientForWin(tx, ctx, proposal.lead);
    const contract = await tx.serviceContract.create({
      data: {
        companyId: ctx.companyId,
        contractNo,
        proposalId: proposal.id,
        clientName: proposal.lead.customerName,
        siteAddress: proposal.siteAddress,
        startDate,
        endDate,
        // Pre-GST. See the warning above.
        annualValue: new Decimal(version.subtotal).toFixed(2),
        frequency,
        visitsPerYear,
        scope: (doc.scope ?? {}) as Prisma.InputJsonValue,
        status: "ACTIVE",
        createdById: ctx.userId,
        visits: { create: schedule.map((d, i) => ({ seq: i + 1, scheduledDate: d, status: "UPCOMING" as const })) },
      },
    });
    await tx.proposal.update({ where: { id: proposal.id }, data: { status: "WON" } });
    await recordProposalOutcome(tx, ctx.companyId, proposal.id, "WON", null); // A14
    await logAudit(
      ctx,
      { action: "APPROVE", entity: "ServiceContract", entityId: contract.id, after: { contractNo, visits: schedule.length } },
      tx,
    );
    return { kind: "CONTRACT" as const, contractId: contract.id, contractNo, visits: schedule.length, already: false };
  });
}

/**
 * A won Service Proposal becomes a ServiceTicket — the job is booked.
 *
 * The ticket carries the quoted `value` (GST-inclusive grand total, since that's what
 * the customer agreed to pay) so won service revenue isn't lost the moment the deal
 * closes. Tickets had no money field at all before this.
 */
async function winAsServiceTicket(
  ctx: Ctx,
  proposal: { id: string; projectName: string; lead: { customerName: string; phone: string; email: string | null; address: string; state: string | null } },
  version: { grandTotal: Prisma.Decimal; documentData: Prisma.JsonValue; technicalText: string | null },
): Promise<MarkWonResult> {
  const { slaDueDate } = await import("@/lib/domain/amc");
  const { asServiceProposalData } = await import("@/lib/domain/proposal-document");
  const doc = asServiceProposalData(version.documentData);
  const priority = doc.priority ?? "MEDIUM";
  const now = new Date();

  return prisma.$transaction(async (tx) => {
    const ticketNo = await allocateNumber(tx, ctx.companyId, "TICKET", now.getFullYear());
    await clientForWin(tx, ctx, proposal.lead);
    const ticket = await tx.serviceTicket.create({
      data: {
        ...
        description: doc.jobDescription || doc.summary || version.technicalText || "Service job won from a proposal.",
        raisedBy: proposal.lead.customerName,
        priority,
        status: "OPEN",
        slaDueDate: slaDueDate(priority, now),
        value: new Decimal(version.grandTotal).toFixed(2),
        createdById: ctx.userId,
      },
    });
    await tx.proposal.update({ where: { id: proposal.id }, data: { status: "WON" } });
    ...
    await logAudit(
      ctx,
      { action: "APPROVE", entity: "ServiceTicket", entityId: ticket.id, after: { ticketNo, value: ticket.value?.toString() } },
      tx,
    );
    return { kind: "TICKET" as const, ticketId: ticket.id, ticketNo, already: false };
  });
}
